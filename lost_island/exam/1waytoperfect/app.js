...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function showAlgorithm(idx) {
    ...
    // Запускаємо генерацію рандомних даних + анімацію + текстовий аналіз
    window.analyzeAlgorithm(algo.name, idx, () => {
  ...
  function showStep() {
    ...
    stepsBlock.innerHTML = `<b>Крок ${currentStep+1}:</b> ${algo.steps[currentStep]}`;
  }

  for (let i = 0; i < 5; ++i) {
    const content = document.querySelector(`#algo-content-${i}`);
    if (!content) continue;
    const ent = content.parentElement;
    ...
    ent.addEventListener("targetLost", () => {
      ...
      // Очищаємо 3D-стовпчики
      const c = document.getElementById(`algo-content-${i}`);
      if (c) c.innerHTML = "";

      // Кажемо візуалізації "стоп", якщо ще щось крутиться
      if (typeof window.cancelArVisualization === "function") {
        window.cancelArVisualization(i);
      }
    });
  }
});
